/**
 * Legacy02: ExtraArgs V2 token transfer (EOA sender).
 *
 * Command to run:
 *   npx hardhat legacy02 --network <NETWORK_NAME> --source-router <SOURCE_ROUTER> --destination-chain-selector <DESTINATION_CHAIN_SELECTOR> --receiver <RECEIVER> --token-to-send <TOKEN_ADDRESS> --amount <AMOUNT> [--gas-limit 200000] [--allow-out-of-order-execution true|false] [--fee-token-address <FEE_TOKEN_ADDRESS>]
 */

import type { HardhatRuntimeEnvironment } from "hardhat/types/hre";
import { decodeEventLog, encodeAbiParameters } from "viem";
import { encodeV2 } from "../scripts/CallEncodeExtraArgsOffchain.js";

export interface Legacy02TaskArguments {
  sourceRouter: string;
...
export default async function legacy02Action(
  taskArguments: Legacy02TaskArguments,
  ...
  console.log("[INFO] Legacy02: ExtraArgsV2 token transfer (EOA sender)");
  ...
  console.log("[INFO] Allow out-of-order execution:", args.allowOutOfOrderExecution);
  console.log("[INFO] Fee token:", feeTokenAddress);
  console.log("[WARN] Use gasLimit 0 if receiver is an EOA.");

  const extraArgs = await encodeV2(args.gasLimit, args.allowOutOfOrderExecution);
  ...
  console.log("[RESULT] Monitor message status at https://ccip.chain.link using message ID above.");
}
