...
import { getPayFeesIn, getPrivateKey, getProviderRpcUrl, getRouterConfig } from "../helpers/utils"
...
task(`deploy-destination-cross-chain-nft-minter`, `Deploys MyNFT.sol and DestinationMinter.sol smart contracts`)
  ...
  .setAction(async (taskArguments: TaskArguments, hre: HardhatRuntimeEnvironment) => {
    ...
    spinner.stop()
    console.log(`✅ MyNFT contract deployed at address ${myNftAddress} on the ${hre.network.name} blockchain`)
    ...
    const destinationMinterFactory: DestinationMinter__factory = (await hre.ethers.getContractFactory(
      "DestinationMinter"
    )) as DestinationMinter__factory
    const destinationMinter: DestinationMinter = await destinationMinterFactory.deploy(routerAddress, myNftAddress)
...
task(`deploy-source-cross-chain-nft-minter`, `Deploys SourceMinter.sol smart contract`)
  ...
  .setAction(async (taskArguments: TaskArguments, hre: HardhatRuntimeEnvironment) => {
    ...
    const sourceMinterFactory: SourceMinter__factory = (await hre.ethers.getContractFactory(
      "SourceMinter"
...
task(`cross-chain-mint`, `Mints the new NFT by sending the Cross-Chain Message`)
  ...
  .setAction(async (taskArguments: TaskArguments) => {
    ...
    const tx = await sourceMinterContract.mint(destinationChainSelector, destinationMinter, fees)
...
task("cross-chain-mint-balance-of", "Gets the balance of MyNFTs for provided address")
  ...
  .setAction(async (taskArguments: TaskArguments) => {
    ...
    spinner.stop()
    console.log(`ℹ️  The balance of MyNFTs of the ${taskArguments.owner} account is ${balanceOf}`)
  })
