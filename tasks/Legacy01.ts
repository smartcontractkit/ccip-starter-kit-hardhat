/**
 * Legacy01: ExtraArgs V1 token transfer (EOA sender).
 *
 * Command to run:
 *   npx hardhat legacy01 --network <NETWORK_NAME> --source-router <SOURCE_ROUTER> --destination-chain-selector <DESTINATION_CHAIN_SELECTOR> --receiver <RECEIVER> --token-to-send <TOKEN_ADDRESS> --amount <AMOUNT> [--gas-limit 200000] [--fee-token-address <FEE_TOKEN_ADDRESS>]
 */

import type { HardhatRuntimeEnvironment } from "hardhat/types/hre";
import { decodeEventLog, encodeAbiParameters } from "viem";
import { encodeV1 } from "../scripts/CallEncodeExtraArgsOffchain.js";

export interface Legacy01TaskArguments {
  sourceRouter: string;
...
export default async function legacy01Action(
  taskArguments: Legacy01TaskArguments,
  ...
  console.log("[INFO] Legacy01: ExtraArgsV1 token transfer (EOA sender)");
  ...
  const extraArgs = await encodeV1(args.gasLimit);
