/**
 * Example07 (CCT 02): Configure LockRelease pool remote lane.
 *
 * Command to run:
 *   npx hardhat example07-step1 --network <NETWORK_NAME> --local-pool <LOCAL_POOL> --remote-chain-selector <REMOTE_CHAIN_SELECTOR> --remote-token <REMOTE_TOKEN> --remote-pool <REMOTE_POOL>
 */

import type { HardhatRuntimeEnvironment } from "hardhat/types/hre";
import { decodeAbiParameters, encodeAbiParameters } from "viem";

export interface Example07Step1TaskArguments {
  localPool: string;
...
export default async function example07Step1Action(
  taskArguments: Example07Step1TaskArguments,
  ...
  const poolArtifact = await hre.artifacts.readArtifact("LockReleaseTokenPool");
  ...
  console.log("[INFO] Example07 (CCT 02): Configure LockRelease pool remote lane");
  ...
  console.log("[RESULT] LockRelease pool lane setup completed for remote chain selector:", remoteChainSelector.toString());
}
