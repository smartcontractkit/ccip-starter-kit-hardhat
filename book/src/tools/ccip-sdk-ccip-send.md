...
Mint one CCIP-BnM token to your keystore account on the source chain with the Hardhat faucet task:

```bash
npx hardhat faucet --network <NETWORK_NAME> --ccip-bnm <CCIP_BNM_TOKEN_ADDRESS_ON_SOURCE_CHAIN>
```

Use the network name that matches your source (e.g. `fuji`, `sepolia`). Repeat as needed to accumulate test tokens.
