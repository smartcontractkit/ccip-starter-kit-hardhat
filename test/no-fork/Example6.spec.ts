...
import { BasicMessageReceiver, BasicMessageSender, CCIPLocalSimulator } from "../../typechain-types"
...
describe("Example 6", function () {
  ...
  async function deployFixture() {
    ...
    const sender: BasicMessageSender = await basicMessageSenderFactory.deploy(config.sourceRouter_, config.linkToken_)

    const basicMessageReceiverFactory = await hre.ethers.getContractFactory("BasicMessageReceiver")
    const receiver: BasicMessageReceiver = await basicMessageReceiverFactory.deploy(config.destinationRouter_)
  ...
  it("Should send and receive cross-chain message and pay for fees in LINK", async function () {
    ...
    await ccipLocalSimulator.requestLinkFromFaucet(senderAddress, 5_000_000_000_000_000_000n)

    const messageToSend = "Hello, World!"

    await sender.send(config.chainSelector_, receiverAddress, messageToSend, PayFeesIn.LINK)

    const [latestMessageId, latestMessageSourceChainSelector, latestMessageSender, latestMessage] =
