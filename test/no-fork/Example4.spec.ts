...
import { BurnMintERC677Helper, CCIPLocalSimulator, ProgrammableTokenTransfers } from "../../typechain-types"
...
describe("Example 4", function () {
  ...
  async function deployFixture() {
    ...
    const sender: ProgrammableTokenTransfers = await programmableTokenTransfersFactory.deploy(config.sourceRouter_)
    const receiver: ProgrammableTokenTransfers = await programmableTokenTransfersFactory.deploy(
  ...
  it("Should test Programmable Token Transfers", async function () {
    ...
    const balanceOfReceiverBefore = await ccipBnMToken.balanceOf(receiverAddress)
    ...
    await sender
      .connect(alice)
      .sendMessage(config.chainSelector_, receiverAddress, messageToSend, await ccipBnMToken.getAddress(), amountToSend)

    const balanceOfSenderAfter = await ccipBnMToken.balanceOf(senderAddress)
    const balanceOfReceiverAfter = await ccipBnMToken.balanceOf(receiverAddress)

    expect(balanceOfSenderBefore - balanceOfSenderAfter).to.equal(amountToSend)
    expect(balanceOfReceiverAfter - balanceOfReceiverBefore).to.equal(amountToSend)
    ...
    expect(latestReceivedMessageDetails.token).to.equal(await ccipBnMToken.getAddress())
