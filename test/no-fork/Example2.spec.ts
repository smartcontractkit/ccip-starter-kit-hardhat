...
describe("Example 2", function () {
  ...
  async function deployFixture() {
    ...
    const basicMessageReceiver: BasicMessageReceiver = await basicMessageReceiverFactory.deploy(
  ...
  it("Should transfer CCIP test tokens from EOA to Smart Contract", async function () {
    ...
    const mockCcipRouter = mockCcipRouterFactory.attach(config.sourceRouter_) as IRouterClient
    ...
    const ccipBnM = ccipBnMFactory.attach(config.ccipBnM_) as BurnMintERC677Helper
    ...
    const message = {
      receiver: defaultAbiCoder.encode(["address"], [basicMessageReceiverAddress]),
    ...
    const linkToken = linkTokenFactory.attach(config.linkToken_) as LinkTokenInterface
    ...
    expect(await ccipBnM.balanceOf(basicMessageReceiverAddress)).to.deep.equal(amountToSend)
  })
})
