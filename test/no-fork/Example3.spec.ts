...
import { BasicTokenSender, BurnMintERC677Helper, CCIPLocalSimulator } from "../../typechain-types"
...
describe("Example 3", function () {
  ...
  async function deployFixture() {
    ...
    const basicTokenSender: BasicTokenSender = await basicTokenSenderFactory.deploy(
      config.sourceRouter_,
      config.linkToken_
    )

    const ccipBnMTokenFactory = await hre.ethers.getContractFactory("BurnMintERC677Helper")
    const ccipBnMToken = ccipBnMTokenFactory.attach(config.ccipBnM_) as BurnMintERC677Helper

    await ccipBnMToken.drip(alice.address)
    const amountToSend = 100n
    await ccipBnMToken.connect(alice).approve(await basicTokenSender.getAddress(), amountToSend)
  ...
  it("Should transfer token(s) from Smart Contract to any destination and pay for fees in LINK", async function () {
    const { ccipLocalSimulator, alice, bob, basicTokenSender, ccipBnMToken, amountToSend, config } =
    ...
    expect(balanceOfAliceBefore - amountToSend).to.deep.equal(balanceOfAliceAfter)
