...
import { CCIPLocalSimulator, DestinationMinter, MyNFT, SourceMinter } from "../../typechain-types"
...
describe("Example 7", function () {
  ...
  async function deployFixture() {
    ...
    const sourceMinter: SourceMinter = await sourceMinterFactory.deploy(config.sourceRouter_, config.linkToken_)
  ...
  it("Should execute received message as a function call", async function () {
    const { ccipLocalSimulator, alice, config, sourceMinter, destinationMinter, myNft } =
      await loadFixture(deployFixture)

    await ccipLocalSimulator.requestLinkFromFaucet(await sourceMinter.getAddress(), 5_000_000_000_000_000_000n)

    await sourceMinter.connect(alice).mint(config.chainSelector_, await destinationMinter.getAddress(), PayFeesIn.LINK)
