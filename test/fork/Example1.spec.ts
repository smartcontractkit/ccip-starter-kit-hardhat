...
import { BurnMintERC677Helper, IRouterClient, IRouterClient__factory, LinkTokenInterface } from "../../typechain-types"
import { getProviderRpcUrl, getLINKTokenAddress, getRouterConfig, getFaucetTokensAddresses } from "../../helpers/utils"
...
describe("Example 1 - Fork", function () {
  it("Should transfer CCIP test tokens from EOA to EOA", async function () {
    ...
    const { address: destinationRouterAddress, chainSelector: destinationChainSelector } = getRouterConfig(destination)
    ...
    const sourceRouter = IRouterClient__factory.connect(sourceRouterAddress, alice) as IRouterClient
    ...
    await sourceCCIPBnM.connect(alice).approve(sourceRouterAddress, amountToSend)
    ...
    const tx = await sourceRouter.connect(alice).ccipSend(destinationChainSelector, message, sourceOverrides)
    ...
    expect(await sourceCCIPBnM.balanceOf(alice.address)).to.deep.equal(aliceBalanceBefore - amountToSend)
    ...
    const destinationBnM = ccipBnMFactory.attach(destinationCCIPBnMTokenAddress) as BurnMintERC677Helper

    expect(await destinationBnM.balanceOf(bob.address)).to.deep.equal(amountToSend)
  })
})
