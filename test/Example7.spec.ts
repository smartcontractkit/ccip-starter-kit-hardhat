...
describe("Example 7", function () {
    ...
    async function deployFixture() {
        ...
        const destinationMinter = await destinationMinterFactory.deploy(config.destinationRouter_, myNft.address);

        await myNft.transferOwnership(destinationMinter.address);

        const sourceMinterFactory = await hre.ethers.getContractFactory("SourceMinter");
        const sourceMinter = await sourceMinterFactory.deploy(config.sourceRouter_, config.linkToken_);

        return { ccipLocalSimulator, alice, config, sourceMinter, destinationMinter, myNft };
    }

    it("Should execute received message as a function call", async function () {
        const { ccipLocalSimulator, alice, config, sourceMinter, destinationMinter, myNft } = await loadFixture(deployFixture);

        await ccipLocalSimulator.requestLinkFromFaucet(sourceMinter.address, 5_000_000_000_000_000_000n);

        await sourceMinter.connect(alice).mint(config.chainSelector_, destinationMinter.address, PayFeesIn.LINK)
