...
describe("Example 3", function () {
    ...
    async function deployFixture() {
        ...
        const basicTokenSender = await basicTokenSenderFactory.deploy(config.sourceRouter_, config.linkToken_);
        ...
        await ccipBnMToken.connect(alice).approve(basicTokenSender.address, amountToSend);

        return { ccipLocalSimulator, alice, bob, basicTokenSender, ccipBnMToken, amountToSend, config };
    }

    it("Should transfer token(s) from Smart Contract to any destination and pay for fees in LINK", async function () {
        const { ccipLocalSimulator, alice, bob, basicTokenSender, ccipBnMToken, amountToSend, config } = await loadFixture(deployFixture);

        await ccipLocalSimulator.requestLinkFromFaucet(alice.address, 5_000_000_000_000_000_000n);
        ...
        await basicTokenSender.connect(alice).send(config.chainSelector_, bob.address, tokenToSendDetails, PayFeesIn.LINK);
        ...
        expect(balanceOfAliceBefore.toBigInt() - amountToSend).to.deep.equal(balanceOfAliceAfter);
        expect(balanceOfBobBefore.toBigInt() + amountToSend).to.deep.equal(balanceOfBobAfter);
    });
    ...
    it("Should transfer token(s) from Smart Contract to any destination and pay for fees in Native coin", async function () {
        ...
        await basicTokenSender.connect(alice).send(config.chainSelector_, bob.address, tokenToSendDetails, PayFeesIn.Native);
